/**
 * 提供了百度坐标（BD09）、国测局坐标（火星坐标，GCJ02）、墨卡托、和WGS84坐标系之间的转换
 */
...
function wgs84togcj02(lng, lat) {
    if (out_of_china(lng, lat)) {
        ...
        dlat = (dlat * 180.0) / ((con_a * (1 - ee)) / (magic * sqrtmagic) * PI);
        dlng = (dlng * 180.0) / (con_a / sqrtmagic * Math.cos(radlat) * PI);
        var mglat = lat + dlat;
        var mglng = lng + dlng;
        return [mglng, mglat];
    }
